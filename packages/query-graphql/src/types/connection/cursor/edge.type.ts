...
import { Class, ValueReflector } from "@franka107-nestjs-query/core";
import {
  ConnectionCursorType,
  ConnectionCursorScalar,
} from "../../cursor.scalar";
...
export function getOrCreateEdgeType<DTO>(
  DTOClass: Class<DTO>
): EdgeTypeConstructor<DTO> {
  return reflector.memoize(DTOClass, () => {
    const objName = getGraphqlObjectName(
      DTOClass,
      "Unable to make EdgeType for class."
    );
    ...
    class AbstractEdge implements EdgeType<DTO> {
      ...
      @Field(() => DTOClass, {
        description: `The node containing the ${objName}`,
      })
      node!: DTO;
