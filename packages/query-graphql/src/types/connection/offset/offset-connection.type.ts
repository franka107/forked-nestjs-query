...
import { Class, MapReflector, Query } from "@franka107-nestjs-query/core";
...
const DEFAULT_COUNT = () =>
  Promise.reject(new NotImplementedException("totalCount not implemented"));

const reflector = new MapReflector("nestjs-query:offset-connection-type");

function getOrCreateConnectionName<DTO>(
  DTOClass: Class<DTO>,
  ...
  const objName = getGraphqlObjectName(
    DTOClass,
    "Unable to make OffsetConnectionType."
  );
  ...
  return reflector.memoize(TItemClass, connectionName, () => {
    ...
    class AbstractConnection implements OffsetConnectionType<DTO> {
      ...
      static async createFromPromise<Q extends Query<DTO>>(
        ...
        const { pageInfo, nodes, totalCount } = await pager.page(
          queryMany,
          query,
          count ?? DEFAULT_COUNT
        );
      ...
      @SkipIf(
        ...
        Field(() => Int, { description: "Fetch total count of records" })
      )
