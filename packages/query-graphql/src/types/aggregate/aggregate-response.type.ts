...
export function AggregateResponseType<DTO>(
  ...
  const objName = getGraphqlObjectName(
    DTOClass,
    "Unable to make AggregationResponseType."
  );
  ...
  return reflector.memoize(DTOClass, aggName, () => {
    ...
    const CountType = NumberAggregatedType(
      `${prefix}CountAggregate`,
      fields,
      Int
    );
    const SumType = NumberAggregatedType(
      `${prefix}SumAggregate`,
      numberFields,
      Float
    );
    const AvgType = NumberAggregatedType(
      `${prefix}AvgAggregate`,
      numberFields,
      Float
    );
    ...
    class AggResponse {
      ...
      @SkipIf(
        ...
        Field(() => SumType, { nullable: true })
      )
      sum?: NumberAggregate<DTO>;

      @SkipIf(
        () => numberFields.length === 0,
        Field(() => AvgType, { nullable: true })
      )
      avg?: NumberAggregate<DTO>;

      @SkipIf(
        () => minMaxFields.length === 0,
        Field(() => MinType, { nullable: true })
      )
      min?: TypeAggregate<DTO>;

      @SkipIf(
        () => minMaxFields.length === 0,
        Field(() => MaxType, { nullable: true })
      )
