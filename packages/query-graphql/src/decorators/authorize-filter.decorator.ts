import { ModifyRelationOptions } from "@franka107-nestjs-query/core";
...
function getRelationAuthFilter<C extends AuthorizerContext<unknown>>(
  ...
  return context.authorizer.authorizeRelation(
    relationName,
    context,
...
export function AuthorizerFilter<DTO>(
  partialAuthContext?: PartialAuthorizationContext
): ParameterDecorator {
  ...
  return (
    ...
    return createParamDecorator(
      (data: unknown, executionContext: ExecutionContext) =>
        getAuthorizerFilter(
          getContext<AuthorizerContext<DTO>>(executionContext),
          authorizationContext
        )
...
export function ModifyRelationAuthorizerFilter<DTO>(
  ...
  return (
    ...
    return createParamDecorator(
      async (
      ...
      ): Promise<ModifyRelationOptions<unknown, unknown>> => {
        ...
        return {
          ...
          relationFilter: await getRelationAuthFilter(
            context,
            relationName,
            authorizationContext
          ),
