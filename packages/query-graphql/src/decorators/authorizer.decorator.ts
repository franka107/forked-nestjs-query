...
export function Authorize<DTO>(
  optsOrAuthorizerOrClass:
    | Class<CustomAuthorizer<DTO>>
    | CustomAuthorizer<DTO>
  ...
  return (DTOClass: Class<DTO>): void => {
    ...
    return reflector.set(
      DTOClass,
      createDefaultAuthorizer(DTOClass, optsOrAuthorizerOrClass)
    );
  };
}

export const getAuthorizer = <DTO>(
  DTOClass: Class<DTO>
): MetaValue<Class<Authorizer<DTO>>> => reflector.get(DTOClass);
export const getCustomAuthorizer = <DTO>(
  DTOClass: Class<DTO>
): MetaValue<Class<CustomAuthorizer<DTO>>> =>
