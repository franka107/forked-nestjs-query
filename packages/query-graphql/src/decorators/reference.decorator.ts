...
export type ReferenceDecoratorOpts<DTO, Relation> = Omit<
  ResolverRelationReference<DTO, Relation>,
  "DTO" | "keys"
>;
...
interface ReferenceDescriptor<DTO, Reference> {
  ...
  relationOpts?: Omit<
    ResolverRelationReference<DTO, Reference>,
    "DTO" | "keys"
  >;
}

function getReferenceDescriptors<DTO>(
  DTOClass: Class<DTO>
): ReferenceDescriptor<DTO, unknown>[] {
  return getPrototypeChain(DTOClass).reduce((references, cls) => {
    const referenceNames = references.map((r) => r.name);
    const metaReferences =
      reflector.get<DTO, ReferenceDescriptor<DTO, unknown>>(
        cls as Class<DTO>
      ) ?? [];
    const inheritedReferences = metaReferences.filter(
...
export function getReferences<DTO>(
  DTOClass: Class<DTO>,
