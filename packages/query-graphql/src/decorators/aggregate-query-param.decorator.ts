...
const QUERY_OPERATORS: (keyof AggregateQuery<unknown>)[] = [
  "groupBy",
  "count",
  "avg",
  "sum",
...
export const AggregateQueryParam = createParamDecorator(
  <DTO>(data: unknown, ctx: ExecutionContext) => {
    const info = GqlExecutionContext.create(ctx).getInfo<GraphQLResolveInfo>();
    const fields = graphqlFields(
      info,
      {},
      { excludedFields: EXCLUDED_FIELDS }
    ) as Record<keyof AggregateQuery<DTO>, Record<keyof DTO, unknown>>;
    return QUERY_OPERATORS.filter((operator) => !!fields[operator]).reduce(
      (query, operator) => {
