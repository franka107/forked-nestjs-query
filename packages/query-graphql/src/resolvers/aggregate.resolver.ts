...
export const Aggregateable =
  ...
  <B extends Class<ServiceResolver<DTO, QS>>>(
    BaseClass: B
  ): Class<AggregateResolver<DTO, QS>> & B => {
    ...
    class AggregateResolverBase extends BaseClass {
      ...
      async aggregate(
        ...
        return this.service.aggregate(
          mergeFilter(qa.filter || {}, authFilter ?? {}),
          query
        );
      }
    }
...
>(
  DTOClass: Class<DTO>,
  opts?: AggregateResolverOpts
): ResolverClass<DTO, QS, AggregateResolver<DTO, QS>> =>
  Aggregateable(DTOClass, opts)(BaseServiceResolver);
