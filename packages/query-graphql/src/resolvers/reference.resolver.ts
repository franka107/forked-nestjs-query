      ...
      async resolveReference(representation: RepresentationType): Promise<DTO> {
        ...
        if (id === undefined) {
          throw new BadRequestException(
            `Unable to resolve reference, missing required key ${key} for ${
              getDTONames(DTOClass).baseName
            }`
          );
        }
...
>(
  DTOClass: Class<DTO>,
  opts: ReferenceResolverOpts = {}
): ResolverClass<DTO, QS, ServiceResolver<DTO, QS>> =>
  Referenceable<DTO, QS>(DTOClass, opts)(BaseServiceResolver);
