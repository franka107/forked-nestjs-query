import { Class, QueryService } from "@franka107-nestjs-query/core";
...
const pluckFields = <DTO, Relation>(
  dto: DTO,
  fieldMap: ReferencesKeys<DTO, Relation>
): Partial<Relation> => {
...
const allFieldsAreNull = <Relation>(fields: Partial<Relation>): boolean => {
  return Object.values(fields).reduce<boolean>(
    (previousNull, value) =>
      previousNull && (value === null || value === undefined),
    true
  );
};
...
const ReferencesMixin =
  <DTO, Relation>(
    DTOClass: Class<DTO>,
    reference: ResolverRelationReference<DTO, Relation>
  ) =>
...
>(
  ...
  ReferencesRelationMixin(DTOClass, references)(BaseServiceResolver);
