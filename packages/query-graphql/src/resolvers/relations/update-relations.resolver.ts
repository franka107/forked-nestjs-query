...
import {
  ModifyRelationAuthorizerFilter,
  ResolverMutation,
} from "../../decorators";
import {
  MutationArgsType,
  ...
  <B extends Class<ServiceResolver<DTO, QueryService<DTO, unknown, unknown>>>>(
    ...
    class UpdateOneMixin extends Base {
      ...
      async [`set${baseName}On${dtoNames.baseName}`](
        ...
        return this.service.setRelation(
          relationName,
          input.id,
          input.relationId,
          modifyRelationsFilter
        );
      }
    }
  ...
  <B extends Class<ServiceResolver<DTO, QueryService<DTO, unknown, unknown>>>>(
    ...
    class UpdateManyMixin extends Base {
      ...
      async [`add${pluralBaseName}To${dtoNames.baseName}`](
        ...
        return this.service.addRelations(
          relationName,
          input.id,
          input.relationIds,
          modifyRelationsFilter
      ...
      async [`set${pluralBaseName}On${dtoNames.baseName}`](
        ...
        const { input } = await transformAndValidate(AddArgs, addArgs);
        return this.service.setRelations(
          relationName,
          input.id,
          input.relationIds,
          modifyRelationsFilter
...
export const UpdateRelationsMixin =
  ...
  <B extends Class<ServiceResolver<DTO, QueryService<DTO, unknown, unknown>>>>(
    ...
    const WithMany = manyRelations.reduce(
      (RB, a) => UpdateManyRelationMixin(DTOClass, a)(RB),
      Base
    );
    return oneRelations.reduce(
      (RB, a) => UpdateOneRelationMixin(DTOClass, a)(RB),
      WithMany
    );
  };
...
>(
  ...
  UpdateRelationsMixin(DTOClass, relations)(BaseServiceResolver);
