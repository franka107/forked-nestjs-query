...
const AggregateRelationMixin =
  <DTO, Relation>(
    DTOClass: Class<DTO>,
    relation: AggregateRelationOpts<Relation>
  ) =>
  <B extends Class<ServiceResolver<DTO, QueryService<DTO, unknown, unknown>>>>(
    ...
    const { baseNameLower, pluralBaseNameLower, pluralBaseName } = getDTONames(
      relationDTO,
      {
    ...
    const aggregateLoader = new AggregateRelationsLoader<DTO, Relation>(
      relationDTO,
      relationName
    );
    @ArgsType()
    ...
    const AR = AggregateResponseType(relationDTO, {
      prefix: `${dtoName}${pluralBaseName}`,
    });
...
export const AggregateRelationsMixin =
  ...
  <B extends Class<ServiceResolver<DTO, QueryService<DTO, unknown, unknown>>>>(
    ...
    return manyRelations.reduce(
      (RB, a) =>
        AggregateRelationMixin(DTOClass, { enableAggregate, ...a })(RB),
      Base
    );
  };
