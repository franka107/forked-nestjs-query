...
import { Class, Filter } from "@franka107-nestjs-query/core";
...
export interface AuthorizerOptions<DTO> {
  ...
  ) => Filter<DTO> | Promise<Filter<DTO>>;
}

const createRelationAuthorizer = (
  opts: AuthorizerOptions<unknown>
): Authorizer<unknown> => ({
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  async authorize(
    context: any,
...
export function createDefaultAuthorizer<DTO>(
  ...
  class DefaultAuthorizer implements Authorizer<DTO> {
    readonly authOptions?: AuthorizerOptions<DTO> | CustomAuthorizer<DTO> =
      opts;
    ...
    constructor(
      ...
      private readonly customAuthorizer?: CustomAuthorizer<DTO>
    ) {
      this.relationsAuthorizers = new Map<
        string,
        Authorizer<unknown> | undefined
      >();
    ...
    async authorizeRelation(
      ...
      if (
        ...
        typeof this.customAuthorizer.authorizeRelation === "function"
      ) {
        const filterFromCustomAuthorizer =
          await this.customAuthorizer.authorizeRelation(
            relationName,
            context,
            authorizationContext
          );
      ...
      if (!this.relationsAuthorizers.has(relationName)) {
        ...
        if (relation.auth) {
          this.relationsAuthorizers.set(
            relationName,
            createRelationAuthorizer(relation.auth)
          );
        } else if (getAuthorizer(relation.DTO)) {
          this.relationsAuthorizers.set(
            relationName,
            this.moduleRef.get(getAuthorizerToken(relation.DTO), {
              strict: false,
            })
          );
        }
    ...
    private getRelations(): Map<string, ResolverRelation<unknown>> {
      ...
      Object.keys(many).forEach((relation) =>
        relationsMap.set(relation, many[relation])
      );
      Object.keys(one).forEach((relation) =>
        relationsMap.set(relation, one[relation])
      );
      return relationsMap;
    }
  }
