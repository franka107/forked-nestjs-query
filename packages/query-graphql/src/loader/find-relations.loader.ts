...
type FindRelationsMap<DTO, Relation> = Map<
  string,
  (FindRelationsArgs<DTO, Relation> & { index: number })[]
>;
    ...
    >
{
  ...
  createLoader(service: QueryService<DTO, unknown, unknown>) {
    return async (
      args: ReadonlyArray<FindRelationsArgs<DTO, Relation>>
    ): Promise<(Relation | undefined | Error)[]> => {
  ...
  private async loadResults(
    ...
    await Promise.all(
      [...findRelationsMap.values()].map(async (args) => {
        ...
        const relationResults = await service.findRelation(
          this.RelationDTO,
          this.relationName,
          dtos,
          { filter }
        );
        const dtoRelations: (Relation | undefined)[] = dtos.map((dto) =>
          relationResults.get(dto)
        );
