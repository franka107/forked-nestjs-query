import { Class, Filter, QueryService } from "@franka107-nestjs-query/core";
import { NestjsQueryDataloader } from "./relations.loader";

type CountRelationsArgs<DTO, Relation> = { dto: DTO; filter: Filter<Relation> };
type CountRelationsMap<DTO, Relation> = Map<
  string,
  (CountRelationsArgs<DTO, Relation> & { index: number })[]
>;
    ...
    >
{
  ...
  createLoader(service: QueryService<DTO, unknown, unknown>) {
    return async (
      queryArgs: ReadonlyArray<CountRelationsArgs<DTO, Relation>>
  ...
  private async loadResults(
    ...
    await Promise.all(
      [...countRelationsMap.values()].map(async (args) => {
        ...
        const relationCountResults = await service.countRelations(
          this.RelationDTO,
          this.relationName,
          dtos,
          filter
        );
        const dtoRelations = dtos.map(
          (dto) => relationCountResults.get(dto) ?? 0
        );
  ...
  private groupQueries(
    countArgs: ReadonlyArray<CountRelationsArgs<DTO, Relation>>
  ): CountRelationsMap<DTO, Relation> {
    // group
