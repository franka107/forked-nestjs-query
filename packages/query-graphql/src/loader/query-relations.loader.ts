import { Class, Query, QueryService } from "@franka107-nestjs-query/core";
import { NestjsQueryDataloader } from "./relations.loader";

type QueryRelationsArgs<DTO, Relation> = { dto: DTO; query: Query<Relation> };
type QueryRelationsMap<DTO, Relation> = Map<
  string,
  (QueryRelationsArgs<DTO, Relation> & { index: number })[]
>;
    ...
    >
{
  ...
  createLoader(service: QueryService<DTO, unknown, unknown>) {
    return async (
      queryArgs: ReadonlyArray<QueryRelationsArgs<DTO, Relation>>
  ...
  private async loadResults(
    ...
    await Promise.all(
      [...queryRelationsMap.values()].map(async (args) => {
        ...
        const relationResults = await service.queryRelations(
          this.RelationDTO,
          this.relationName,
          dtos,
          query
  ...
  private groupQueries(
    queryArgs: ReadonlyArray<QueryRelationsArgs<DTO, Relation>>
  ): QueryRelationsMap<DTO, Relation> {
    // group
