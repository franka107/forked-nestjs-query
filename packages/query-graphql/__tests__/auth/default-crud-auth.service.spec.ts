...
describe("createDefaultAuthorizer", () => {
  ...
  @Injectable()
  class TestWithAuthorizerAuthorizer
    implements CustomAuthorizer<TestWithAuthorizerDTO>
  {
  ...
  it("should create an auth filter", async () => {
    const authorizer = testingModule.get<Authorizer<TestDTO>>(
      getAuthorizerToken(TestDTO)
    );
  ...
  it("should return an empty filter if auth not found", async () => {
    const authorizer = testingModule.get<Authorizer<TestNoAuthDTO>>(
      getAuthorizerToken(TestNoAuthDTO)
    );
    const filter = await authorizer.authorize(
      { user: { id: 2 } },
      {
  ...
  it("should call authorizeRelation of authorizer and fallback to authorize decorator", async () => {
    const authorizer = testingModule.get<Authorizer<TestWithAuthorizerDTO>>(
      getAuthorizerToken(TestWithAuthorizerDTO)
    );
    jest.spyOn(authorizer, "authorizeRelation");
    const customAuthorizer = testingModule.get<
      CustomAuthorizer<TestWithAuthorizerDTO>
  ...
  it("should call authorizeRelation of authorizer and fallback to custom authorizer of relation", async () => {
    ...
    jest.spyOn(customAuthorizer, "authorizeRelation");
    expect(customAuthorizer).toBeDefined();
    const relationAuthorizer = testingModule.get<
      Authorizer<RelationWithAuthorizer>
    >(getAuthorizerToken(RelationWithAuthorizer));
    jest.spyOn(relationAuthorizer, "authorize");
    const customRelationAuthorizer = testingModule.get<
      CustomAuthorizer<RelationWithAuthorizer>
