...
import { Filter, UpdateManyResponse } from "@franka107-nestjs-query/core";
...
describe("UpdateResolver", () => {
  const expectResolverSDL = async (
    opts?: UpdateResolverOpts<TestResolverDTO>
  ) => {
  ...
  it("should use the dtoName if provided", () =>
    expectResolverSDL({ dtoName: "Test" }));
  ...
  it("should use the UpdateDTOClass if provided", () =>
    expectResolverSDL({ UpdateDTOClass: TestResolverInputDTO }));

  it("should not expose update methods if disabled", () =>
    expectResolverSDL({ disabled: true }));
  ...
  describe("#updateOne", () => {
    ...
    it("should not expose update one method if disabled", () =>
      expectResolverSDL({ one: { disabled: true } }));
    ...
    it("should call the service updateOne with the provided input", async () => {
      ...
      when(
        mockService.updateOne(
          input.id,
          objectContaining(input.update),
          deepEqual({ filter: {} })
        )
      ).thenResolve(output);
    ...
    it("should call the service updateOne with the provided input and optional auth filter", async () => {
      ...
      const authorizeFilter: Filter<TestResolverDTO> = {
        stringField: { eq: "foo" },
      };
      when(
        mockService.updateOne(
          input.id,
          objectContaining(input.update),
          deepEqual({ filter: authorizeFilter })
        )
      ).thenResolve(output);
  ...
  describe("#updateMany", () => {
    it("should not update a new type if the UpdateManyArgs is supplied", () => {
      @InputType()
      class CustomUpdateManyInput extends UpdateManyInputType(
        TestResolverDTO,
        TestResolverInputDTO
      ) {
        @Field()
    ...
    it("should not expose update many method if disabled", () =>
      expectResolverSDL({ many: { disabled: true } }));

    it("should call the service updateMany with the provided input", async () => {
      const { resolver, mockService } = await createTestResolver();
      const input: MutationArgsType<
        UpdateManyInputType<TestResolverDTO, Partial<TestResolverDTO>>
      > = {
        input: {
          filter: { id: { eq: "id-1" } },
          update: {
      ...
      when(
        mockService.updateMany(
          ...
          objectContaining(input.input.filter)
        )
      ).thenResolve(output);
  ...
  describe("updated subscription", () => {
    ...
    describe("update one events", () => {
      it("should publish events for create one when enableSubscriptions is set to true for all", async () => {
        ...
        const eventName = getDTOEventName(
          EventType.UPDATED_ONE,
          TestResolverDTO
        );
    ...
    describe("updatedOneSubscription", () => {
      it("should propagate events if enableSubscriptions is true", async () => {
        ...
        const mockIterator =
          mock<AsyncIterator<UpdatedEvent<TestResolverDTO>>>();
        when(mockPubSub.asyncIterator(eventName)).thenReturn(
          instance(mockIterator)
        );
      ...
      it("should not propagate events if enableSubscriptions is false", async () => {
        ...
        return expect(() => resolver.updatedOneSubscription()).toThrow(
          `Unable to subscribe to ${eventName}`
        );
      });
    ...
    describe("updatedManySubscription", () => {
      ...
      it("should not propagate events if enableSubscriptions is false", async () => {
        ...
        return expect(() => resolver.updatedManySubscription()).toThrow(
          `Unable to subscribe to ${eventName}`
        );
      });
