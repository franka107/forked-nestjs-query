...
} from "../../src/resolvers/aggregate.resolver";
import {
  generateSchema,
...
describe("AggregateResolver", () => {
  ...
  it("should create a AggregateResolver for the DTO", () =>
    expectResolverSDL({ enabled: true }));
  ...
  describe("#aggregate", () => {
    @Resolver(() => TestResolverDTO)
    class TestResolver extends AggregateResolver(TestResolverDTO, {
      enabled: true,
    }) {
    ...
    it("should call the service query with the provided input", async () => {
      ...
      when(
        mockService.aggregate(
          objectContaining(input.filter!),
          deepEqual(aggregateQuery)
        )
      ).thenResolve(output);
