...
describe("FederationResolver", () => {
  const generateSDL = <DTO extends TestResolverDTO>(
    DTOClass: Class<DTO>
  ): Promise<string> => {
    @Resolver(() => DTOClass)
  ...
  describe("many - connection", () => {
    describe("with cursor paging strategy", () => {
      it("should call the service findRelation with the provided dto", async () => {
        ...
        // @ts-ignore
        // eslint-disable-next-line @typescript-eslint/no-unsafe-call
        const result = await resolver.queryRelationCursorConnections(
          dto,
          query,
          {}
        );
  ...
  describe("with offset paging strategy", () => {
    it("should call the service findRelation with the provided dto", async () => {
      ...
      // @ts-ignore
      // eslint-disable-next-line @typescript-eslint/no-unsafe-call
      const result = await resolver.queryRelationOffsetConnections(
        dto,
        query,
        {}
      );
