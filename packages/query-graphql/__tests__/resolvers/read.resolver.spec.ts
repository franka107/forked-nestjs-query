...
describe("ReadResolver", () => {
  const expectResolverSDL = async (
    opts?: ReadResolverOpts<TestResolverDTO>
  ) => {
  ...
  describe("query many", () => {
    ...
    describe("#queryMany cursor connection", () => {
      ...
      it("should call the service query with the provided input", async () => {
        const { resolver, mockService } = await createResolverFromNest(
          TestResolver
        );
      ...
      it("should merge the filter an auth filter if provided", async () => {
        ...
        when(
          mockService.query(
            objectContaining({
              filter: { ...input.filter, ...authorizeFilter },
      ...
      it("should call the service count", async () => {
        ...
        const result = await resolver.queryMany(input);
        when(mockService.count(objectContaining(input.filter!))).thenResolve(
          10
        );
      ...
      it("should call the service count with the provided input and auth filter", async () => {
        ...
        when(
          mockService.count(
            objectContaining({ ...input.filter!, ...authorizeFilter })
          )
        ).thenResolve(10);
        return expect(result.totalCount).resolves.toBe(10);
      });
  ...
  describe("#findById", () => {
    ...
    it("should call the service findById with the provided input", async () => {
      ...
      const context = {};
      when(
        mockService.findById(input.id, deepEqual({ filter: {} }))
      ).thenResolve(output);
    ...
    it("should call the service findById with the provided input filter and authFilter", async () => {
      ...
      when(
        mockService.findById(input.id, deepEqual({ filter: authorizeFilter }))
      ).thenResolve(output);
