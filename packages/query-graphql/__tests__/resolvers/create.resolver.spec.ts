...
describe("CreateResolver", () => {
  const expectResolverSDL = async (
    opts?: CreateResolverOpts<TestResolverDTO>
  ) => {
  ...
  it("should use the CreateDTOClass if provided", () =>
    expectResolverSDL({ CreateDTOClass: TestResolverInputDTO }));
  ...
  describe("#createOne", () => {
    it("should use the provided CreateOneInput type", () => {
      @InputType()
      class CreateOneInput extends CreateOneInputType(
        "createResolverDTO",
        TestResolverInputDTO
      ) {}
    ...
    it("should call the service createOne with the provided input", async () => {
      ...
      when(mockService.createOne(objectContaining(args.input))).thenResolve(
        output
      );
  ...
  describe("#createMany", () => {
    ...
    it("should call the service createMany with the provided input", async () => {
      ...
      when(mockService.createMany(objectContaining(args.input))).thenResolve(
        output
      );
  ...
  describe("created subscription", () => {
    ...
    describe("create many events", () => {
      it("should publish events for create many when enableSubscriptions is set to true for all", async () => {
        ...
        events.forEach((e) =>
          when(mockPubSub.publish(eventName, deepEqual(e))).thenResolve()
        );
        const result = await resolver.createMany({ input: args });
        events.forEach((e) =>
          verify(mockPubSub.publish(eventName, deepEqual(e))).once()
        );
        return expect(result).toEqual(output);
      });
    ...
    describe("createSubscription", () => {
      it("should propagate events if enableSubscriptions is true", async () => {
        ...
        const mockIterator =
          mock<AsyncIterator<CreatedEvent<TestResolverDTO>>>();
      ...
      it("should not propagate events if enableSubscriptions is false", async () => {
        ...
        const eventName = getDTOEventName(EventType.CREATED, TestResolverDTO);
        return expect(() => resolver.createdSubscription()).toThrow(
