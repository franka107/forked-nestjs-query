import { AggregateQuery, QueryService } from "@franka107-nestjs-query/core";
...
describe("AggregateRelationsLoader", () => {
  describe("createLoader", () => {
    ...
    it("should return a function that accepts aggregate args", () => {
      ...
      const queryRelationsLoader = new AggregateRelationsLoader(
        RelationDTO,
        "relation"
      );
      expect(
        queryRelationsLoader.createLoader(instance(service))
      ).toBeInstanceOf(Function);
    });

    it("should try to load the relations with the query args", () => {
      const service = mock<QueryService<DTO>>();
      const aggregateRelationsLoader = new AggregateRelationsLoader(
        RelationDTO,
        "relation"
      ).createLoader(instance(service));
      const filter = {};
