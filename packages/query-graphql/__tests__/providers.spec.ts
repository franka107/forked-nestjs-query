...
describe("createTypeOrmQueryServiceProviders", () => {
  ...
  describe("entity crud resolver", () => {
    it("should create a provider for the entity", () => {
      const providers = createResolvers([
        { DTOClass: TestDTO, EntityClass: TestDTO },
      ]);
      expect(providers).toHaveLength(1);
      const Provider = providers[0] as Class<
        CRUDResolver<TestDTO, TestDTO, TestDTO, ReadResolverOpts<TestDTO>>
      >;
      expect(Provider.name).toBe("TestDTOAutoResolver");
      expect(new Provider(NoOpQueryService.getInstance())).toBeInstanceOf(
        Provider
      );
    });
    ...
    it("should create a federated provider for the entity", () => {
      ...
      const providers = createResolvers([
        { type: "federated", DTOClass: TestDTO, Service },
      ]);
      expect(providers).toHaveLength(1);
      const Provider = providers[0] as Class<
        ServiceResolver<TestDTO, QueryService<TestDTO>>
      >;
