import { DeepPartial } from "@franka107-nestjs-query/core";
...
import {
  ...
  refresh,
  truncate,
} from "../__fixtures__/sequelize.fixture";
import {
  PLAIN_TEST_ENTITIES,
  PLAIN_TEST_RELATIONS,
...
describe("SequelizeQueryService", (): void => {
  ...
  class TestEntityService extends SequelizeQueryService<TestEntity> {
    constructor(
      @InjectModel(TestEntity) readonly model: ModelCtor<TestEntity>
    ) {
      super(model);
    }
  }
  ...
  class TestRelationService extends SequelizeQueryService<TestRelation> {
    constructor(
      @InjectModel(TestRelation) readonly model: ModelCtor<TestRelation>
    ) {
      super(model);
    }
  }
  ...
  describe("#query", () => {
    it("call select and return the result", async () => {
      ...
      return expect(queryResult.map((e) => e.get({ plain: true }))).toEqual([
        PLAIN_TEST_ENTITIES[0],
      ]);
    });
    describe("filter on relations", () => {
      describe("oneToOne", () => {
        it("should allow filtering on a one to one relation", async () => {
          ...
          expect(queryResult.map((e) => e.get({ plain: true }))).toEqual([
            entity,
          ]);
        });
      });
              ...
              testEntity: {
                testEntityPk: {
                  in: [
                    ...
                    PLAIN_TEST_ENTITIES[1].testEntityPk,
                  ],
                },
              },
            },
          });
          expect(queryResults.map((e) => e.get({ plain: true }))).toEqual(
            PLAIN_TEST_RELATIONS.slice(0, 6)
          );
        });
      });
          ...
          const queryResult = await queryService.query({
            filter: {
              testRelations: {
                relationName: {
                  in: [
                    PLAIN_TEST_RELATIONS[0].relationName,
                    PLAIN_TEST_RELATIONS[1].relationName,
                  ],
                },
              },
            },
          ...
          const count = await queryService.count({
            testEntity: {
              testEntityPk: {
                in: [
                  PLAIN_TEST_ENTITIES[0].testEntityPk,
                  PLAIN_TEST_ENTITIES[2].testEntityPk,
                ],
              },
            },
          });
  ...
  describe("#queryRelations", () => {
    ...
    describe("with multiple entities", () => {
      it("call select and return the result", async () => {
        const entities = PLAIN_TEST_ENTITIES.slice(0, 3).map((pe) =>
          TestEntity.build(pe)
        );
        const queryService = moduleRef.get(TestEntityService);
        const queryResult = await queryService.queryRelations(
  ...
  describe("#findRelation", () => {
    describe("with one entity", () => {
      it("call select and return the result", async () => {
        ...
        expect(queryResult!.get({ plain: true })).toEqual(
          PLAIN_TEST_RELATIONS[0]
        );
      });
      ...
      it("apply the filter option", async () => {
        ...
        const queryResult1 = await queryService.findRelation(
          ...
          {
            filter: {
              relationName: { eq: PLAIN_TEST_RELATIONS[0].relationName },
            },
          }
        );
        expect(queryResult1!.get({ plain: true })).toEqual(
          PLAIN_TEST_RELATIONS[0]
        );
        ...
        const queryResult2 = await queryService.findRelation(
          ...
          {
            filter: {
              relationName: { eq: PLAIN_TEST_RELATIONS[1].relationName },
            },
          }
        );
      ...
      it("throw an error if a relation with that name is not found.", async () => {
        ...
        const entity = TestEntity.build(PLAIN_TEST_ENTITIES[0]);
        return expect(
          queryService.findRelation(TestRelation, "badRelation", entity)
            ...
            filter: {
              testRelationPk: {
                in: [
                  ...
                  PLAIN_TEST_RELATIONS[6].testRelationPk,
                ],
              },
            },
          }
  ...
  describe("#addRelations", () => {
    it("call select and return the result", async () => {
      ...
      const queryResult = await queryService.addRelations(
        "testRelations",
        entity.testEntityPk,
        PLAIN_TEST_RELATIONS.slice(3, 6).map(
          (r) => (r as TestRelation).testRelationPk
        )
      );
  ...
  describe("#setRelations", () => {
    it("set all relations on the entity", async () => {
      const entity = PLAIN_TEST_ENTITIES[0] as TestEntity;
      const queryService = moduleRef.get(TestEntityService);
      const relationIds = PLAIN_TEST_RELATIONS.slice(3, 6).map(
        (r) => r.testRelationPk
      );
  ...
  describe("#setRelation", () => {
    it("call select and return the result", async () => {
      ...
      expect(relation!.testRelationPk).toBe(
        PLAIN_TEST_RELATIONS[1].testRelationPk
      );
    });
  ...
  describe("#removeRelation", () => {
        ...
        it("should throw an error if the entity is not found with the id and provided filter", async () => {
          ...
          return expect(
            queryService.removeRelation(
              ...
              {
                filter: {
                  stringType: { eq: PLAIN_TEST_ENTITIES[1].stringType },
                },
              }
            )
          ).rejects.toThrow("Unable to find TestEntity with id: test-entity-1");
    ...
    describe("manyToOne", () => {
      it("set the relation to null", async () => {
        ...
        expect(queryResult).toEqual(
          expect.objectContaining({ ...relation, testEntityId: null })
        );
  ...
  describe("#createMany", () => {
    it("call save on the repo with instances of entities when passed plain objects", async () => {
      ...
      expect(created.map((c) => c.get({ plain: true }))).toEqual(
        expect.objectContaining(PLAIN_TEST_ENTITIES)
      );
    });
    ...
    it("call save on the repo with instances of entities when passed instances", async () => {
      ...
      const created = await queryService.createMany(
        entities as DeepPartial<TestEntity>[]
      );
      expect(created.map((c) => c.get({ plain: true }))).toEqual(
        PLAIN_TEST_ENTITIES
      );
    ...
    it("should reject if the entities already exist", async () => {
      const queryService = moduleRef.get(TestEntityService);
      return expect(
        queryService.createMany(PLAIN_TEST_ENTITIES)
  ...
  describe("#createOne", () => {
    ...
    it("call save on the repo with an instance of the entity when passed an instance", async () => {
      ...
      const entity = PLAIN_TEST_ENTITIES[0];
      const queryService = moduleRef.get(TestEntityService);
      const created = await queryService.createOne(
        entity as DeepPartial<TestEntity>
      );
      expect(created).toEqual(expect.objectContaining(entity));
    });
  ...
  describe("#deleteMany", () => {
    it("delete all records that match the query", async () => {
      ...
      const { deletedCount } = await queryService.deleteMany({
        testEntityPk: {
          in: PLAIN_TEST_ENTITIES.slice(0, 5).map((e) => e.testEntityPk),
        },
      });
    ...
    describe("with filter", () => {
      it("should delete the entity if all filters match", async () => {
        ...
        expect(deleted).toEqual(
          expect.objectContaining(PLAIN_TEST_ENTITIES[0])
        );
      });
  ...
  describe("#updateOne", () => {
    ...
    it("should reject if the update contains a primary key", async () => {
      const queryService = moduleRef.get(TestEntityService);
      return expect(
        queryService.updateOne(PLAIN_TEST_ENTITIES[0].testEntityPk, {
          testEntityPk: "bad-id",
        })
      ).rejects.toThrow("Id cannot be specified when updating");
    });
