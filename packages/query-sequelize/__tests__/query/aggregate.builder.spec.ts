...
import { AggregateQuery } from "@franka107-nestjs-query/core";
...
describe("AggregateBuilder", (): void => {
  ...
  const createAggregateBuilder = () =>
    new AggregateBuilder<TestEntity>(TestEntity);

  const expectAggregateQuery = (
    agg: AggregateQuery<TestEntity>,
    expected: Projectable
  ): void => {
  ...
  it("should throw an error if no selects are generated", (): void => {
    expect(() => createAggregateBuilder().build({})).toThrow(
  ...
  it("should create selects for all aggregate functions", (): void => {
    expectAggregateQuery(
      ...
      },
      {
        attributes: [
          [
            sequelize.fn("COUNT", sequelize.col("test_entity_pk")),
            "COUNT_testEntityPk",
          ],
          [
            sequelize.fn("COUNT", sequelize.col("string_type")),
            "COUNT_stringType",
          ],
