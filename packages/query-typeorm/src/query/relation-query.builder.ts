...
import { AggregateQuery, Class, Query } from "@franka107-nestjs-query/core";
import {
  Repository,
  SelectQueryBuilder,
  ObjectLiteral,
...
export class RelationQueryBuilder<Entity, Relation> {
  ...
  select(entity: Entity, query: Query<Relation>): SelectQueryBuilder<Relation> {
    ...
    relationBuilder = hasRelations
      ? this.filterQueryBuilder.applyRelationJoinsRecursive(
          relationBuilder,
          this.filterQueryBuilder.getReferencedRelationsRecursive(
  ...
  batchSelect(
    ...
    const unionedBuilder = this.relationRepo
      ...
      .addSelect(
        `${this.escapedUnionAlias}.${this.escapedEntityIndexColName}`,
        this.entityIndexColName
      )
      .innerJoin(
        `(${unionFragment.sql})`,
        this.unionAlias,
        unionFragment.joinCondition,
        unionFragment.params
      );
    return this.filterQueryBuilder.applySorting(
      unionedBuilder.addOrderBy(
        `${this.escapedUnionAlias}.${this.escapedEntityIndexColName}`,
        "ASC"
      ),
      query.sorting,
  ...
  private createUnionSelectSubQuery(
    ...
    const joinCondition = fromPrimaryKeys
      .map(
        (fpk) =>
          `${fpk.selectPath} = ${this.escapedUnionAlias}.${this.escapeName(
            `${fromAlias}_${fpk.databasePath}`
          )}`
      )
      .join(" AND ");
  ...
  getManyToOneOrOneToOneOwnerMeta(
    ...
    const joins: JoinColumn[] = [
      {
        ...
        conditions: joinColumns.map((joinColumn) => ({
          ...
          rightHand: `${relation.propertyName}.${
  ...
  getOneToManyOrOneToOneNotOwnerMeta(
    relation: RelationMetadata
  ): RelationQuery<Relation, Entity> {
