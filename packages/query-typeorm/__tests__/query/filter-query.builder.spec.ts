...
describe("FilterQueryBuilder", (): void => {
  ...
  describe("#update", () => {
    const expectUpdateSQLSnapshot = (
      query: Query<TestEntity>,
      whereBuilder: WhereBuilder<TestEntity>
    ): void => {
      const queryBuilder = getEntityQueryBuilder(TestEntity, whereBuilder)
        .update(query)
        .set({ stringType: "baz" });
